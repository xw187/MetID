...
get_cleaned <- function(filename, type = c('data.frame','csv','txt'), na, sep){
  ...
  if (type == 'data.frame'){
    df <- filename
      ...
      df <- read.csv(filename, header = T, stringsAsFactors = F, na.strings = na)
  } else if (type == 'txt'){
        df <- read.table(filename, header = T, stringsAsFactors = F, na.strings = na)
  } else {
  ...
  ## Format data
  df_formated <- data.frame(metid,qmass,emass,kid,cid,inchikey,stringsAsFactors = F)
  # remove NAs
